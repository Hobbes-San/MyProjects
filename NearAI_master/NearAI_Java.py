...
from pytorch_tools_master.pytorch_tools import torchfold
...
shuffle(java_data); java_data = java_data[:3000]
...
class RecursiveModel(nn.Module):
    ...
    def process_children(self, left_h, left_c, right_h, right_c):
        lstm_in = self.left(left_h) + self.right(right_h)
        a, i, f1, f2, o = lstm_in.chunk(5, 1)
        c = a.tanh() * i.sigmoid() + f1.sigmoid()*left_c + f2.sigmoid()*right_c
        h = o.sigmoid() * c.tanh()
...
java_fold = torchfold.Fold(); #Cpp_fold = torchfold.Fold()
